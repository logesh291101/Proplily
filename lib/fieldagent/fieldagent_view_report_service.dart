...
sealed class FieldAgentViewReportFetchResult {
  const FieldAgentViewReportFetchResult();
}

final class FieldAgentViewReportFetchSuccess
    extends FieldAgentViewReportFetchResult {
  const FieldAgentViewReportFetchSuccess(this.model);

  final FieldAgentViewReportModel model;
}

final class FieldAgentViewReportFetchFailure
    extends FieldAgentViewReportFetchResult {
  const FieldAgentViewReportFetchFailure({
...
/// Fetches submitted report from
/// `GET {live_url}/coordinator_api/tasks/{task_id}/report`.
class FieldAgentViewReportService {
  FieldAgentViewReportService({http.Client? httpClient})
      : _httpClient = httpClient;

  final http.Client? _httpClient;

  Future<FieldAgentViewReportFetchResult> fetchReport(String taskId) async {
    final trimmedTaskId = taskId.trim();
    if (trimmedTaskId.isEmpty) {
      return const FieldAgentViewReportFetchFailure(
    ...
    try {
      ...
      return FieldAgentViewReportFetchFailure(
      ...
      return const FieldAgentViewReportFetchFailure(
        message: 'Could not read report data. Please try again.',
      );
    } catch (e) {
      return FieldAgentViewReportFetchFailure(
  ...
  FieldAgentViewReportFetchResult _parseSuccessBody(String body) {
    ...
    final model = FieldAgentViewReportModel.fromJson(decoded);

    final apiError = _readErrorsText(model.errors);
    if (apiError != null) {
      return FieldAgentViewReportFetchFailure(message: apiError);
    }

    if (!model.isSuccess || model.data == null) {
      final msg = model.message?.trim() ?? '';
      return FieldAgentViewReportFetchFailure(
        message: msg.isNotEmpty ? msg : 'Could not load report.',
      );
    }

    return FieldAgentViewReportFetchSuccess(model);
  }
  ...
  String _messageForStatus(int statusCode, String body) {
    ...
    switch (statusCode) {
      ...
      default:
        return 'Could not load report ($statusCode).';
    }
  }
