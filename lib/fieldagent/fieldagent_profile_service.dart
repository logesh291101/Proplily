...
sealed class FieldAgentProfileFetchResult {
  const FieldAgentProfileFetchResult();
}

final class FieldAgentProfileFetchSuccess extends FieldAgentProfileFetchResult {
  const FieldAgentProfileFetchSuccess(this.model);

  final FieldAgentProfileModel model;
}

final class FieldAgentProfileFetchFailure extends FieldAgentProfileFetchResult {
  const FieldAgentProfileFetchFailure({required this.message});

  final String message;
}

/// Fetches field agent profile from `GET {live_url}/api/profile`.
class FieldAgentProfileService {
  FieldAgentProfileService({http.Client? httpClient}) : _httpClient = httpClient;

  final http.Client? _httpClient;

  Future<FieldAgentProfileFetchResult> fetchProfile() async {
    final prefs = await SharedPreferences.getInstance();
    final token =
        prefs.getString(AuthPreferenceKeys.token)?.trim() ?? '';

    if (token.isEmpty) {
      return const FieldAgentProfileFetchFailure(
    ...
    try {
      ...
      return FieldAgentProfileFetchFailure(
  ...
  FieldAgentProfileFetchResult _parseSuccessBody(String body) {
    ...
    final model = FieldAgentProfileModel.fromJson(decoded);

    final apiError = _readErrorsText(model.errors);
    if (apiError != null) {
      return FieldAgentProfileFetchFailure(message: apiError);
    }
    ...
    return FieldAgentProfileFetchSuccess(model);
  }
