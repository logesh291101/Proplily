...
final class LoginSuccess extends LoginResult {
  const LoginSuccess({this.message, required this.role});
...
class LoginService {
  ...
  Future<LoginResult> login({
    ...
    try {
      ...
      if (_isLoginSuccess(parsed, response.statusCode)) {
        final persistResult = await _persistLoginData(prefs, parsed);
        ...
        return LoginSuccess(
          message: _readMessageField(parsed),
          role: persistResult,
        );
      }
  ...
  /// Returns the normalized [role] when persistence succeeds.
  Future<String?> _persistLoginData(
